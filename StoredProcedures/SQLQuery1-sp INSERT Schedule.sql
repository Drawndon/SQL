...
--		IF EXISTS	(SELECT	holiday FROM DaysOFF WHERE [date] = @date) CONTINUE;
		...
		--Больше не нужен DECLARE	@day	AS		TINYINT	=	DATEPART(WEEKDAY, @date); --Как раз для этого выше написано 'SET DATEFIRST 1'
		--PRINT(@day);
		--SET	@date		=	DATEADD(DAY, IIF(@day=5, 3, 2), @date); Это только для нашего расписания
		--SET		@date		=	dbo.GetNextLearningDate(@group_name, @date);
END
